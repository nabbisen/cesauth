//! `GET /admin/tenancy/tenants/:tid/organizations/new` — create-organization form.
//!
...
pub fn organization_create_form(
    ...
    let body = format!(
        r##"<p><a href="{back_url}">← Back to tenant detail</a></p>
{error}
...
<section aria-label="Help" class="muted">
  ...
  <ul>
    ...
    <li>Quota: this tenant's plan caps the number of organizations.
        ...
        back_url   = escape(&routes::tenant(&tenant.id)),
        action_url = escape(&routes::tenant_orgs_new(&tenant.id)),
        error = match error {
            None => String::new(),
            Some(msg) => format!(
...
mod tests {
    ...
    fn form_action_includes_tenant_id() {
        ...
        assert!(html.contains(r#"action="/admin/tenancy/tenants/t-acme/organizations/new""#));
    }
