...
async fn recover_normal_path_returns_success_and_consumes_code_and_challenge() {
    ...
    // Challenge consumed.
    assert!(store.peek(&totp_handle(), 0).await.unwrap().is_none(),
...
async fn recover_csrf_mismatch_returns_csrf_failure_and_does_not_consume_challenge() {
    ...
    assert!(store.peek(&totp_handle(), 0).await.unwrap().is_some(),
...
// =====================================================================
// RFC 140 test 6 — an expired PendingTotp gate is absent to recover.
// =====================================================================

#[tokio::test]
async fn rfc140_recover_rejects_an_expired_gate() {
    let (_, recovery_repo) = fixture_with_pending_totp_and_codes(&["X"]).await;
    let store = InMemoryAuthChallengeStore::default();
    ...
    // A valid recovery code, presented at expiry.
    let decision = decide_recover_post(&form, &cookie, "X", TOTP_HANDLE, &store, &recovery_repo, 100).await;
    assert!(matches!(decision, RecoverDecision::NoChallenge));
    assert!(store.peek(&totp_handle(), 0).await.unwrap().is_none(), "the expired take deleted it");
}
