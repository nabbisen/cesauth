...
pub async fn authorize<D>(req: Request, ctx: RouteContext<D>) -> Result<Response> {
    ...
    // v0.79.7: The login page now serves the Leptos shell.  Instead of
    // rendering the login template directly, redirect the browser to
    // /login with the pending-authorize cookie already set.  The Leptos
    // Login component completes authentication and resumes the AR.
    let mut resp = Response::empty()?.with_status(302);
    ...
    log::emit(&cfg.log, Level::Debug, Category::Auth,
        "authorize cold path - AR parked, redirect to /login", Some(&ar.client_id));
    Ok(resp)
}
