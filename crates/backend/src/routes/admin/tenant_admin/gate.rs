...
pub async fn build_affordances<D>(
    ctx_ta: &TenantAdminContext,
    ctx:    &RouteContext<D>,
) -> Result<cesauth_frontend::tenant_admin::Affordances> {
    use cesauth_cf::authz::{CloudflareRoleAssignmentRepository, CloudflareRoleRepository};
    ...
    let user_id = match &ctx_ta.principal.user_id {
        Some(id) => id.as_str(),
        None     => return Ok(cesauth_frontend::tenant_admin::Affordances::default()),
    };
    ...
    Ok(cesauth_frontend::tenant_admin::Affordances {
