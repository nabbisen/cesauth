//! `GET/POST /admin/tenancy/tenants/:tid/subscription/status`.
...
use cesauth_frontend::tenancy_console::forms::subscription_set_status::{confirm_page, form_page};
...
pub async fn submit<D>(mut req: Request, ctx: RouteContext<D>) -> Result<Response> {
    ...
    audit::write_owned(
        ...
        Some(format!("via=tenancy-console,from={:?},to={:?}",
