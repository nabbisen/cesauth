//! `GET/POST /admin/tenancy/tenants/:tid/organizations/new` — organization create.
...
use cesauth_frontend::tenancy_console::forms::organization_create::organization_create_form;
...
pub async fn submit<D>(mut req: Request, ctx: RouteContext<D>) -> Result<Response> {
    ...
    audit::write_owned(
        ...
        Some(format!("via=tenancy-console,tenant={tid},slug={}", org.slug)),
    ).await.ok();

    redirect_303(&format!("/admin/tenancy/organizations/{}", org.id))
}
