//! `GET/POST /admin/tenancy/groups/:gid/delete` — group soft delete.

use cesauth_cf::tenancy::CloudflareGroupRepository;
use cesauth_core::tenancy::ports::GroupRepository;
use cesauth_frontend::tenancy_console::forms::group_delete::confirm_page;
...
pub async fn submit<D>(mut req: Request, ctx: RouteContext<D>) -> Result<Response> {
    ...
    audit::write_owned(
        ...
        Some(format!("via=tenancy-console,slug={}", group.slug)),
    ).await.ok();
