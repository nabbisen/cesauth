...
use cesauth_frontend::tenancy_console::forms::role_assignment_delete::confirm_page;
...
async fn role_label(env: &worker::Env, role_id: &str) -> String {
    ...
    match roles.get(role_id).await.ok().flatten() {
        Some(r) => format!(
            "{name} (<code>{slug}</code>)",
            name = cesauth_frontend::escape(&r.display_name),
            slug = cesauth_frontend::escape(&r.slug),
        ),
        None => format!("<code>{}</code>", cesauth_frontend::escape(role_id)),
    }
}
