//! # cesauth-backend
//!
//! The deployment-facing crate: HTTP routing, authentication flows,
//! OIDC flows, admin console, and post-auth orchestration.
//!
//! The crate is deployment-neutral at the module level.  The
//! Cloudflare Workers entrypoint lives in
//! `entrypoints::cloudflare`; a future self-hosted entrypoint
//! would be `entrypoints::axum`.
//!
//! ## DO re-export dance
//!
...
//! `cesauth-adapter-cloudflare` (a `rlib`), we re-export them via
//! `entrypoints::cloudflare` so the exports land in *this* crate's
//! WASM module. `wrangler.toml`'s DO bindings point at these
//! re-exported names.
...
// DO re-exports are in entrypoints::cloudflare; re-export here so they
// land at the cdylib root where the #[durable_object] WASM exports
// are expected.  Do NOT remove without updating wrangler.toml and
// verifying DO classes still load.
pub use entrypoints::cloudflare::{ActiveSession, AuthChallenge, RateLimit, RefreshTokenFamily};
...
mod security_headers {
    ...
    /// per-request `CspNonce`, sets it on the `cesauth_frontend` renderer
    /// via `cesauth_frontend::set_render_nonce`, and injects it into the
    ...
    /// This function reads the nonce back from `cesauth_frontend::render_nonce()`
    ...
    pub fn apply(mut resp: Response, env: &Env) -> Response {
        ...
        let nonce_str = cesauth_frontend::render_nonce();
