//! `GET/POST /admin/saas/role_assignments/:id/delete` — revoke a
...
use cesauth_ui::saas::forms::role_assignment_delete::confirm_page;
...
pub async fn submit<D>(mut req: Request, ctx: RouteContext<D>) -> Result<Response> {
    ...
    audit::write_owned(
        ...
        Some(id.clone()),
        Some(format!("via=saas-console,user={user_id}")),
    ).await.ok();

    redirect_303(&format!("/admin/saas/users/{user_id}/role_assignments"))
}
