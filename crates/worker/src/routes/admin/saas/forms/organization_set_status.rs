//! `GET/POST /admin/saas/organizations/:oid/status` — organization status.
...
use cesauth_ui::saas::forms::organization_set_status::{confirm_page, form_page};
...
pub async fn submit<D>(mut req: Request, ctx: RouteContext<D>) -> Result<Response> {
    ...
    audit::write_owned(
        ...
        Some(format!("via=saas-console,target={:?},reason={}",
            target, reason).to_lowercase()),
    ).await.ok();

    redirect_303(&format!("/admin/saas/organizations/{oid}"))
}
