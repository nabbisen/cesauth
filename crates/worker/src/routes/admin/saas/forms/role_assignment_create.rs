//! `GET/POST /admin/saas/users/:uid/role_assignments/new` — grant role.
...
use cesauth_ui::saas::forms::role_assignment_create::{
...
pub async fn submit<D>(mut req: Request, ctx: RouteContext<D>) -> Result<Response> {
    ...
    audit::write_owned(
        ...
        Some(format!("via=saas-console,user={uid},role={role_id}")),
    ).await.ok();

    redirect_303(&format!("/admin/saas/users/{uid}/role_assignments"))
}
