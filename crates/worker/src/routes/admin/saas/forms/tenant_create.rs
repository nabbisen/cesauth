//! `GET/POST /admin/saas/tenants/new` — tenant create form.
...
use cesauth_ui::saas::forms::tenant_create::tenant_create_form;
...
pub async fn submit<D>(mut req: Request, ctx: RouteContext<D>) -> Result<Response> {
    ...
    audit::write_owned(
        ...
        Some(format!("via=saas-console,slug={}", tenant.slug)),
    ).await.ok();

    redirect_303(&format!("/admin/saas/tenants/{}", tenant.id))
}
