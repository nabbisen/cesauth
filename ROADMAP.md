...
| **Session hardening v0.35.0** | ✅       | Shipped 0.35.0. ADR-012 Accepted. Closes the five gaps a v0.34.x audit against the session BCP guidance surfaced. **Idle timeout** — `Config::session_idle_timeout_secs` (default 30 min, `SESSION_IDLE_TIMEOUT_SECS` env, 0 disables); `ActiveSessionStore::touch` extended to take both `idle_timeout_secs` and `absolute_ttl_secs` and consult both gates atomically with the touch update inside the DO (no race window between peek and revoke). `SessionStatus` gains `IdleExpired(SessionState)` + `AbsoluteExpired(SessionState)` variants populated with `revoked_at = Some(now)` before the DO returns. Order: absolute gate consulted FIRST (deeper-cause attribution; pinned by `session_touch_absolute_takes_priority_over_idle`). **Auth resolver wired to touch()** — load-bearing change: `me::auth::resolve_or_redirect` switched from `sessions.status()` to `sessions.touch(session_id, now, idle, absolute)`. Without this the new gates would be dormant — `touch()` was the only writer of `last_seen_at` and was never being called from production v0.34.x. This also fixes a structural bug where `last_seen_at` never advanced past `created_at`. v0.34.x sessions idle for >30 min when v0.35.0 deploys will be revoked on their next request (BCP-correct behavior; operators should communicate to users if they expect long-idle sessions). **Per-user index** — new D1 migration `0009_user_session_index.sql` (SCHEMA_VERSION 8 → 9): `user_sessions` table (session_id PRIMARY KEY, user_id INDEX, created_at, revoked_at, auth_method, client_id) + `user_sessions_user_idx ON (user_id, created_at DESC)`. Per-session DO remains source of truth; D1 is denormalized index for user-facing list. `ActiveSessionStore::list_for_user(user_id, include_revoked, limit)` port method; in-memory adapter does O(n) scan, CF adapter SELECTs from D1 (DO not consulted in this path; hot path unchanged). `start()` writes both DO + D1 INSERT; `touch()` returning Idle/AbsoluteExpired triggers best-effort D1 mirror; `revoke()` writes DO + D1 UPDATE. Eventually-consistent: D1 mirror failure does NOT unwind authoritative DO write. `last_seen_at` and `scopes` deliberately NOT mirrored to D1 (hot-path mutable; mirroring would multiply write pressure for no user-visible benefit; list page renders `created_at` instead). ADR-012 §"Considered alternatives" documents why D1 over a second `UserSessionIndex` DO type. **`/me/security/sessions` page** — renders one card per active session (auth method label パスキー / Magic Link / 管理者ログイン, sign-in time, last access time, client_id, shortened session id) with "取り消す" per row. Current session shows "この端末" badge + disabled button (revoking your own session via this page would cause next request to bounce to /login — surprising UX; user should use explicit logout flow). Cross-linked from Security Center. `POST /me/security/sessions/:session_id/revoke` handler: CSRF guard, refuses revoking current session (defensive — UI button disabled for that row), refuses revoking another user's session via 403 (ownership check; defense in depth even though page only renders caller's sessions). **Audit event split** — `EventKind` gains `SessionRevokedByUser`, `SessionRevokedByAdmin`, `SessionIdleTimeout`, `SessionAbsoluteTimeout`. Resolver emits timeout kinds when `touch()` returns Idle/AbsoluteExpired (best-effort, audit failure does not block redirect). User revoke handler emits `SessionRevokedByUser` with payload `{session_id, revoked_by:"user", actor_user_id}`. Legacy `SessionRevoked` retained for backward-compatibility with v0.4-v0.34.x audit chain rows. **Session-id rotation on login** was already correct in v0.34.x via `complete_auth_post_gate` minting `Uuid::new_v4` per successful auth — preserved unchanged. **"New device" notification deferred** to a future release: requires general transactional email infrastructure cesauth doesn't yet have (only Magic Link email plumbing exists), would entangle two changes. **Tests**: 783 → **801** (+18). 11 in adapter-test (`session_touch_active_bumps_last_seen`, `session_touch_idle_window_expired_revokes_atomically`, `session_touch_idle_disabled_when_zero`, `session_touch_absolute_lifetime_expires_regardless_of_activity`, `session_touch_absolute_takes_priority_over_idle`, `session_touch_already_revoked_is_idempotent`, `session_touch_unknown_returns_not_started`, `session_list_for_user_returns_only_that_user_newest_first`, `session_list_for_user_excludes_revoked_by_default`, `session_list_for_user_respects_limit`, `session_list_for_user_empty_when_no_sessions`). 7 in cesauth-ui sessions_page tests covering empty / multiple-rows / current-marked / non-current revoke form / unknown-method-fallback / flash-splice / session_id-escaping. **Schema migration required**: operators MUST run `wrangler d1 migrations apply` before serving v0.35.0 traffic. Without the migration `start()` fails on D1 INSERT and new sessions can't be created. Existing sessions continue working through touch/revoke (which only write DO + best-effort D1; the D1 write failing during the transition window is acceptable since no entry exists to update). **Open Questions Q1-Q4** documented in ADR-012 for future scheduling: per-tenant idle override, revoke-all-other-sessions button, auto-revoke flash with i18n key, D1 user_sessions retention sweep. |
...
- **Audit log integrity track.**
  ...
  - **0.34.0** ✅ — Refresh token reuse hardening shipped
    ...
    **Wire format unchanged** for OAuth clients. 777 → **783**
    ...
    (per-family rate limiting at `/token`, user notification
    on reuse, tenant admin aggregate view).
