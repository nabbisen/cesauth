...
- **UI/UX iteration release.**
  ...
  - **0.31.1** — TOTP handler integration tests (P1-B split
    from v0.31.0). The four route handlers
    (`enroll::get_handler`, `enroll::post_confirm_handler`,
    `verify::get_handler`/`post_handler`,
    `recover::post_handler`, `disable::post_handler`) were
    written in v0.27.0 - v0.30.0 and have only had pure-helper
    coverage (template tests, library function tests, a few
    boundary-decision extracts in v0.31.0). The split was
    invoked because adding env-mock infrastructure
    (`worker::Env` is opaque to the host runtime; faking D1,
    DO, KV, secrets, vars requires substantial scaffolding)
    would have inflated v0.31.0 well past the review-able
    slice that the TOTP-track 5-release split established.
    v0.31.1 is the place to do that scaffolding investment
    once and use it for all four handlers. Acceptance: each
    handler has at least 3 cases pinning normal / CSRF
