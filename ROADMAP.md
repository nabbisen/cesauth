...
- **Internationalization (i18n) track.** **Status: not yet
  ...
  - **i18n-2** 🚧 partial in v0.36.0 — initial migration of
    high-visibility surfaces: flash banners (5 keys),
    `/me/security/sessions` chrome (16 keys: page title,
    intro, empty state, back link, current-device badge +
    disabled button + title, revoke button, four
    auth-method labels, four session-meta labels), TOTP
    enroll wrong-code error. **5 UI rendering tests**
    pinning English under `Locale::En` and the JA default
    shorthand. Remaining surfaces (login, TOTP page chrome,
    magic link, security center index) deferred to v0.36.1+.
  - **i18n-2** continued (post-v0.36.0) — login page chrome
    (handle/password labels, "send magic link" button etc.),
    TOTP enroll/verify/disable/recover page chrome, magic
    link request/verify pages, security center index page
    (TOTP/sessions section headers + intro paragraphs).
    Audit + admin console stay English (operator-facing).
